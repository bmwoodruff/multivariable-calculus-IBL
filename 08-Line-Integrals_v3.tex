...
\note{ I wish we could keep more use a computer to integrate... but that requires deeper thinking about how to structure the course... : SoTL Idea : have students work on collaborative tablets/note-books so can use technology directly. }


\noindent 
This unit covers the following ideas. \valposhort{In preparation for the concept checks and exam, make sure you have a review packet containing examples that explain and illustrate the following concepts.}  \bmw{In preparation for the quiz and exam, make sure you have a lesson plan containing examples that explain and illustrate the following concepts.}  
\begin{enumerate}
...
\bmw{If you would like homework exercises from the text that line up with the ideas we are studying, please use the following tables.

{\noindent \footnotesize 
\begin{tabular}{|l|c|l|l|l|l|}\hline
Topic (11th ed.) &Sec &Basic Practice &Good exercises &Thy/App &Comp \\\hline
...
{\noindent \footnotesize 
\begin{tabular}{|l|c|l|l|l|l|}\hline
Topic (12th ed.) &Sec &Basic Practice &Good exercises &Thy/App &Comp \\\hline
...
Do I need to start with area, or would it be better to start with work.  I'm thinking work might be a better way to start.  Then I can put random work exercises in the middle (just to beef up our abilities).
Add flux midway through.

Start with work.  Then talk about area. Then talk about flux. Then talk about average value and center of mass.  Leave out inertia for now (add it in later - this removes 5 exercises, an entire day.  They just have to be added later on, and the quizzes and exams have to be updated.)  
...
exercises 8.1 and 8.2 have to be shrunk.  They cannot be that long. They really are short, so make them short.  Get rid of all the extra fluff, and put it later. 
...
Let's start with simple review. Recall that the work done by a vector field $\vec F$ through a displacement $\vec d$ is the dot product $\vec F\cdot \vec d$. We saw this in Exercise \ref{first work problem}. 
\begin{review*}
...
\begin{enumerate}
	\item \marginpar{See exercise \ref{horse track chain rule introduction}}%
...
\note{We commented out a few exercises here to cut down on the exercises in this chapter.  They were originally put here to practice (a) parametrizing curves, (b) basic integration techniques, and (c) computing line integrals.  We'll try to practice these things in the exercises in the rest of the chapter, though.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%IMPORTANT. SEE THE NOTE BELOW.
\note{There are 3 exercises below that I used the first semester, but not the second.  I think it went way better when I included these exercises.  They are really fast (just plug in numbers and compute), but they help the students get the hand of line integrals.  I suggest including them next time. I'll probably include these in the exact same exercise, just have parts a,b,c.}
...
\begin{challenge}\marginpar{If you've forgotten how to parametrize line segments, see \ref{prob:line equation practice}.}%
\marginpar{\href{\sagelineintegral}{Check your answer with Sage.}}%
 Let $f(x,y,z)=x^2+3yz$. Let $C$ be the straight line segment from $(1,0,0)$ to $(0,4,5)$. Compute $\int_C f ds$. 
\end{challenge}

\begin{problem}\marginpar{See \ref{prob:parametric curve in plane} if you forgot how to parametrize plane curves.}%
\larsonfive{\marginpar{See Larson 15.2: 1--20, 63--70.  Some exercises give you a parametrization, some expect you to come up with one on your own.}}%
\marginpar{\href{\sagelineintegral}{Check your answer with Sage.}}%
...
\begin{problem}\marginpar{See \href{http://aleph.sagemath.org/?z=eJxNjEEOQDAUBfdO8j-vSZWtrUs0ZUFFo0FopL09VmwnM9NSROKGVB5FQuRs91voLzuE7egnZ_1ILR5HKJQMSpComQvvVktaP1Qa6BKVMQizG5bVnmejuHg3VIvYqa_-CzduCCO3}{Sage}. Think of an airplane wing as you solve this exercise.}%
...
\note{In both of the preceding exercises, both the circulation and flux are positive.  Everything is always positive. This would give a student a false impression that work and flux are always positive.  We need to change some computations.  Make sure to look into this. It needs to change.}
...
\begin{problem}
\marginpar{\href{http://www.youtube.com/watch?v=8Tk2pEIOnwg&list=PL04DF68E73B7ECD54&index=9&feature=plpp_video}{Watch a YouTube Video}.}%
Let's practice finding gradients and potentials. \newline [Hint: For 1 and 2 the first derivative you take should yield a 1x2 vector (matrix).]
\begin{enumerate}
...
\begin{challenge}[Proof of the Fundamental Theorem]\marginpar{The proof of the fundamental theorem of line integrals is quite short. All you need is the fundamental theorem of calculus, together with the chain rule (\ref{chain rule def}).}
...
[If you do this exercise, skip exercise \ref{prob:proof of fundamental thm2} ] 
\end{challenge}

\begin{problem}[Guided Proof of Fundamental Theorem]\label{prob:proof of fundamental thm2}
...
[Hint: If you parametrize the curve, then you'll have done the exercise the HARD way. You don't need any parametrization at all.]
\begin{enumerate}
...
\end{enumerate}
 [Hint 2: Did you find a potential, and then plug in the end points?]
\end{problem}
...
%If a vector field is everywhere continuously differentiable, then the last exercise shows that conservative vector fields and gradient fields are exactly the same.
...
%Because of timing of an exam, I needed to do day 4 (gradients) on a 2nd day, we didn't really get to them. So I took the original set of exercises and cut them down some to be about half a day...
...
%\begin{enumerate}
 ...
 %Let $\vec F(x,y,z) = (2x+yz,2z+xz,2y+xy)$ and $C$ be the straight segment from $(2,-5,0)$ to $(1,2,3)$. 
%\end{enumerate}
%[Hint: If you parametrize the curve, then you've done the exercise the HARD way. You don't need any parameterizations at all. Did you find a potential, and then plug in the end points?]
%\end{problem}
%
%\begin{problem}\marginpar{See \href{http://aleph.sagemath.org/?z=eJytkM1uhCAUhfc-hTsueMmMYNOV23mJydQYpR1SK0TItPr0hVFbm5kumnQBl597zvngUg9APKHJAT5wxImWoU440sRY78pWNx78WTevvXKulDSxpe2Mry6q8WaonrXqWtnCAdOg4zkKGlYj8gILijAhl_hIgyorbT3Ub8oPuqmiA0BjHPjQ7nR_rXGAxz0KZjVFxiLBVdrpXsER8q-7FAQWKOkJ07UtzkfSmM4M5FSSQbVkI53bgy567Le6e2SCr0wFmykZ5Dzu5bJfUa3eiR-ov3oxwfzO6tVSsm-7B_YPhuFgLllnXiDPIrPns92SEzPnhzyJP-RlMe-qvX3CbWa-4G0-a3N4P9adzTtYmnwCd8u7lQ}{Sage}---$C_1$ and $C_2$ are in blue, and several possible $C_3$ are shown in red.}%
  %Let $\vec F = (x,z,y)$. Let $C_1$ be the curve which starts at $(1,0,0)$ and follows a helical path $(\cos t, \sin t, t)$ to $(1,0,2\pi)$. Let $C_2$ be the curve which starts at $(1,0, 2\pi)$ and follows a straight line path to $(2,4,3)$. Let $C_3$ be any smooth curve that starts at $(2,4,3)$ and ends at $(0,1,2)$.
...
Compute the numerator of the fraction in the previous part by multiplying each score by how many times it occurs, rather than adding it in the sum that many times.  Again, write down the calculation for $\bar s$ before doing any arithmetic.
  \item \marginpar{$\bar s=\sum (\text{value}\cdot\text{(\% of stuff)})=\sum (\text{value} \cdot \frac{\text{weight}}{\text{\# of values}} ) $}%
...
$$\int_a^b \bar f dx = \int_a^b f dx$$
  Solve for $\bar f$ symbolically (without doing any of the integrals). This quantity is called the average value of $f$ over $[a,b]$.\newline [Hint: Remember $\bar{f}$ is a constant]%, and was crucial to proving the Fundamental Theorem of Calculus from first-semester calculus.
...
\begin{enumerate}
	\item Draw the surface $f$ in 3D.
		...
		\end{enumerate}
  \item What's the maximum height and minimum height of the sheet? \marginpar{See exercise \ref{horse track chain rule introduction}.}
...
\instructor{Again, I emphasize how this relates to the ways of computing averages from Exercise~\ref{average value methods}.}
...
\begin{problem}\instructor{After this exercise, I like to emphasize that they should have noticed a linear growth rate, and then I show them how I would have guessed the answer.}%
...
\begin{problem}[Centroid]\label{prob:centroid of curve}\marginpar{\href{http://www.youtube.com/watch?v=t7T0MzfgV0Q&list=PL04DF68E73B7ECD54&index=5&feature=plpp_video}{Watch a YouTube video.}}%
  Notice the word ``average'' in the definition of the centroid. Use the concept of average value to explain why the coordinates of the centroid are %the formulas below. [Hint:  If we have a curve $C$ with parametrization $\vec r(t)$ and function $f$ so that $f(\vec r(t))$ is continuous, then we've developed in Exercise~\ref{Average Value intro} a formula for the average value of $f$ along $C$.  What function $f(x,y,z)$ gives the $x$-coordinate of a point?]
...
\item Find the centroid of two objects.
	...
	\begin{enumerate}[a)]
		...
		[Hint: Think of it as a weighted average. If you are still stuck, see the margin]
...
\begin{problem}
This exercise reinforces what you just did with two points in the previous exercise. However, it now involves two people on a seesaw. \marginpar{See \href{http://en.wikipedia.org/wiki/Seesaw}{Wikipedia} for a seesaw picture.}%
...
\begin{challenge}
\instructor{I purposefully put this exercise in to show students how to generalize a formula from prime numbers to any number.  I mention how I use primes (5, 7, 11, 13) when I'm looking for a pattern.  I want to help them develop this skill a little. However, if you are pressed for time, then skip 5.}%
