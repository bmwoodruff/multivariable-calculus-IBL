...
\noindent 
This unit covers the following ideas. \valposhort{In preparation for the quiz and exam, make sure you have a review packet containing examples that explain and illustrate the following concepts.  }\bmw{In preparation for the quiz and exam, make sure you have a lesson plan containing examples that explain and illustrate the following concepts.} 
\begin{enumerate}
...
\bmw{The following homework exercises line up with the topics we will discuss in class.
...
\instructor{ Original Day 1 start}
...
\end{itemize}
 [Hint: If you need a reminder of how to draw level curves look back at exercise \ref{prob:intro to contour plots} ]
\begin{enumerate} 
...
\end{enumerate}
\note{This exercise might be better if on part 3 we wrote, 
...
\begin{challenge}
The two surfaces $x^2+y^2+z^2=14$ and $3x+4y-z=-1$ intersect in a curve $C$. Draw both surfaces, and show us the curve $C$. Then, at the point $(2,-1,3)$, find an equation of the tangent line to this curve.\newline [Hint: The line is in both tangent planes, so it is orthogonal to both normal vectors. The cross product gets you a vector that is orthogonal to two vectors.]
\end{challenge}

% \begin{problem}\note{This is almost exactly like \ref{estimating change with differentials} and feels redundant. NOTE: I added this exercise because it is a pet favorite of mine.  I decided to remove it.  I may add it back next semester as an optional exercise.  It's not exactly easy to see the answer, whereas it is simple to see the solution without differentials on the volume exercise.}
...
%  \item Why do you know the ant must have reached both a maximum and minimum temperature?
  \item \marginpar{The most common error on this exercise is to divide both sides of an equation by $x$, which could be zero.  If you do this, you'll only get 2 ordered pairs.}%
...
\note{We finished all the exercises in this unit in 4 days of time (really only 3 and a half, as the line integral unit overlapped on the first day.}
