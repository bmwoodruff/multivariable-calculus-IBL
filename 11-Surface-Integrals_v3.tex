...
\begin{enumerate}[(a)]
	\item sketch a plot (you can graph it in Sage or WolframAlpha first) \marginpar{See 
...
\href{http://wolfr.am/y4gIgX}{Wolfram Alpha} to plot the vector fields}
...
Find the curl\\ \marginpar{See \href{http://mathinsight.org/curl_components}{MathInsight.org} for a really nice animation of the curl of a vector field.} for each vector field:
\begin{enumerate}
\item $\vec{F}(x,y)=(x,y)$
\item $\vec{F}(x,y)=(y,x)$
\item $\vec{F}(x,y)=(x,x+y)$
...
\note{Karl: I turned this on and edited it to reflect that I added a whole separate section which introduces the div and curl directly, with practice. }
...
\note{Karl: I'm not entirely happy with the mix of definitions/explanation given earlier in the curl section. Here the curl is motivated/explained more, but requires limits etc. Which I skipped earlier when defining the curl. }

%------------
%This set of text/explanation, including the definition, do not appear anywhere
...
%=====================================-------------
%The text below is included earlier in the Div/Curl section.
